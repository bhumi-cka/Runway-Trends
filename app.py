...
# Generate all feature labels for similarity detection
labels = []
for component in labels1['components']:
    if component!='top' and component!='shirt':
...
# Create flat list of all features from labels1 for the dropdown
all_features_flat = []
for category, items in labels1.items():
    all_features_flat.extend(items)
...
<body>
    ...
    <script>
        ...
        // Features data from labels1
        const featuresData = {{ labels1|tojson }};
        
        // Create flat list of all features for searching
        const allFeatures = [];
        Object.values(featuresData).forEach(categoryItems => {
            allFeatures.push(...categoryItems);
        });
        ...
        function updateFeaturesDropdown() {
            ...
            if (query === '') {
                ...
                Object.entries(featuresData).forEach(([category, items]) => {
        ...
        document.getElementById('features-input').addEventListener('keydown', function(event) {
            ...
            if (event.key === 'ArrowDown') {
                ...
                if (highlightedIndex >= 0 && options[highlightedIndex]) {
                    const featureText = options[highlightedIndex].textContent;
                    selectFeature(featureText);
                }
